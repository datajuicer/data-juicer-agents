...
def test_apply_event_updates_tool_running_and_done_status():
    ...
    assert call.tool == "plan_generate"
    assert "intent" in call.args_preview
    assert not state.timeline
    assert state.status_line == "Running plan_generate"
    ...
    assert "plan generated" in call.summary
    assert state.timeline[-1].status == "done"
    assert "Finished plan_generate" in state.timeline[-1].title
