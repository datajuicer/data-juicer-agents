...
关键参数：
- `--dataset`：可选数据集路径；提供后，CLI 会通过 `inspect_dataset_schema` 探测数据集模态（text / image / multimodal / audio / video），并将其转换为算子标签用于过滤
...
基于数据集的过滤：
- 当提供 `--dataset` 时，CLI 调用 `inspect_dataset_schema` 检测数据集模态
